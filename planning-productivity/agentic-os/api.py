    ...
    def _galaxy(self):
        ...
        for root, dirs, filenames in os.walk(vault):
            for fn in filenames:
                ...
                # Create slug from filename (no extension)
                slug = os.path.splitext(fn)[0]
                title = slug.replace('-', ' ').replace('_', ' ')

                # Get file modification time for "recent glow"
                try:
                ...
                nodes.append({
                    ...
                    'tags': tags[:5],  # cap tags
                    'folder': os.path.dirname(rel) if os.path.dirname(rel) else 'root',
                })

        # Second pass: resolve links between notes
        for root, dirs, filenames in os.walk(vault):
            for fn in filenames:
                if not fn.endswith('.md'):
                    continue
                ...
                wikilinks = re.findall(r'\[\[([^\]|]+)(?:\|[^\]]+)?\]\]', content)
                src_idx = slug_map.get(slug)
                if src_idx is None:
                    continue
                for link_target in wikilinks:
                    # Normalize: remove anchors, take first part of aliases
                    clean = link_target.split('#')[0].strip()
